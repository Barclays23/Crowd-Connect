...
// Overlay – uses your modal overlay variable
...
const sheetVariants = cva(
  // base styles – replaced Tailwind background/shadow with your CSS variables
  "fixed z-50 gap-4 bg-[var(--modal-content-bg)] p-6 shadow-[var(--modal-shadow)] transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      {/* Close button – adapted to your theme */}
      <SheetPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-[var(--bg-primary)] transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-[var(--brand-primary-light)] focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-[var(--bg-secondary)]">
        <X className="h-4 w-4" />
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title
    ref={ref}
    className={cn(
      "text-lg font-semibold text-[var(--heading-primary)]",
      className
    )}
    {...props}
  />
