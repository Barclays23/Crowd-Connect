...
function EventsDiscoveryPage() {
   ...
   // Calculate local date limits (Min: Today, Max: 1 Year from today)
   const { todayStr, maxDateStr } = React.useMemo(() => {
      const today = new Date();
      
      const maxDate = new Date();
      maxDate.setFullYear(today.getFullYear() + 1); // For 6 months, use: maxDate.setMonth(today.getMonth() + 6)

      return {
         todayStr: today.toLocaleDateString('en-CA'),
         maxDateStr: maxDate.toLocaleDateString('en-CA')
      };
   }, []);
   ...
   const clearFilters = () => {
      ...
      setLocationResetKey(prev => prev + 1); // Forces the location input to remount and clear
      setStartDate("");
      setEndDate("");
      setCurrentPage(1);
      fetchEvents(1, "");
                     ...
                     <div className="flex-1 min-w-0 lg:px-3 lg:py-1">
                        ...
                        <div className="relative">
                           ...
                           <GooglePlacesAutoComplete
                              key={`loc-search-${locationResetKey}`}
