...
export class UserManagementService implements IUserManagementService {
    ...
    async createUserByAdmin({createDto, imageFile, currentAdminId}: {
        ...
        // Inform the user their account was created and provide the temp password and instructions to change it
    ...
    async editUserByAdmin({targetUserId, currentAdminId, updateDto, imageFile}: {
        ...
        if (imageFile){
            ...
            if (targetUser.profilePic && targetUser.profilePic.trim() !== '') {
                try {
                    await this._storageService.deleteFile(targetUser.profilePic, 'image');

                } catch (cleanupErr: unknown) {
                    const msg = cleanupErr instanceof Error ? cleanupErr.message : 'Storage deletion failed';
                    console.warn("Failed to delete user profile pic from storage:", msg);
                }
            }
        }
    ...
    async deleteUser({ targetUserId, currentAdminId }: { 
        ...
        if (targetUser.profilePic && targetUser.profilePic.trim() !== '') {
            try {
                ...
                console.warn("Failed to delete user profile pic from storage:", cleanupErr);
            }
        }
        ...
        if (targetUser.certificateUrl && targetUser.certificateUrl.trim() !== '') {
            try {
                await this._storageService.deleteFile(targetUser.certificateUrl, 'image');
